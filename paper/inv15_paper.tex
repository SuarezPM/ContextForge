...
\author{Pablo M. Suarez\thanks{Correspondence:
        ...
        Author profile:
        \href{https://www.researchgate.net/profile/Pablo-Suarez-23}%
             {ResearchGate} \textperiodcentered{}
        \href{https://github.com/SuarezPM}{GitHub @SuarezPM}.}\\
...
\begin{equation}
  \Risk(\mathbf{x}) =
  \min\!\Big(1,\;
  b(\rho) + \alpha_n\,\mathbb{1}\!\left[\Cand \geq 3\right]
         + \alpha_s\,\Shuf
         + \alpha_u\,\mathbb{1}\!\left[\Reuse > 0.8\right]
  \Big),
  \label{eq:risk}
\end{equation}
where
...
\begin{algorithm}[t]
\caption{\GateDecision: \INV{15}-enforcing preflight}
\label{alg:gate}
\begin{algorithmic}[1]
\Require agent role $\rho$, candidate count $\Cand$,
         reuse rate $\Reuse$, layout-shuffled flag $\Shuf$
...
\State \texttt{log}.\textsc{append}$((\rho, \Cand, \Reuse, \Shuf, r))$
\If{$\rho \in \Judge$ \textbf{and} $r > \tau$}
  \State \Return \textsc{true}\quad\textit{// \INV{15} fires}
\Else
