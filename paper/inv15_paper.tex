...
\begin{table}[t]
  ...
  \caption{End-to-end metrics on MI300X (DevCloud ATL1, 2026-05-10;
           ...
           appended). Numbers in bold are the \INV{15}-relevant subset.
           S-11's honest deviation under the V5 toy ramp is reported
           openly; the V6.2 row gives the same controller under a
           statistically rigorous Poisson load.}
  \label{tab:e2e}
  \begin{tabular}{@{}lr@{}}
    \toprule
    Metric & Value \\
    \midrule
    Benchmark scenarios passed (V6.1, honest)   & $14 / 15$ \\
    Adversarial scenarios passed (V6.2)         & $6 / 6$ \\
    Unit tests passed                           & $310 / 310$ \\
    Failed tests                                & $0$ \\
    \midrule
    \textbf{\INV{15} violations (1{,}210-pt sweep)} & $\mathbf{0}$ \\
    \textbf{Critic dense prefill rate}          & $\mathbf{0.851}$ \\
    \textbf{Gate throughput}                    & $\mathbf{6\!\times\!10^{5}\,\mathrm{decisions/s}}$ \\
    \textbf{Mean gate latency}                  & $\mathbf{1.7\,\mathrm{\mu s}}$ \\
    \midrule
    TokenDance compression                      & $10.81\times$ \\
    TokenDance reconstruction error             & $1.19\times10^{-7}$ \\
    M/G/1 $\lambda_{\text{crit}}$ deviation (V6.2) & $1.6\,\%\!-\!29.9\,\%$ \\
